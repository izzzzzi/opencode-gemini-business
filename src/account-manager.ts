...
export class AccountManager {
  ...
  needsJWTRefresh(account: GeminiBusinessAccount): boolean {
    if (!account.jwt || !account.jwt_expires) return true;

    const now = Math.floor(Date.now() / 1000);
    const expiresIn = account.jwt_expires - now;

    return expiresIn < this.config.jwt_refresh_threshold;
  }
  ...
  async updateJWT(accountId: string, jwt: string, expiresIn: number): Promise<void> {
    ...
    account.jwt = jwt;
    account.jwt_expires = Math.floor(Date.now() / 1000) + expiresIn;
    await this.saveAccounts();
  }
